#ifndef MEDROPS_GP_HPP
#define MEDROPS_GP_HPP
...
*/
        ...
        Eigen::VectorXd max_observation() const
        {
            if (_observations.cols() > 1)
                std::cout << "WARNING max_observation with multi dimensional "
                          ...
                          << std::endl;
            return limbo::tools::make_vector(_observations.maxCoeff());
        }
    ...
    protected:
        ...
        void _compute_full_kernel()
        {
            ...
            _kernel.resize(n, n);
            _noise = std::exp(2.0 * _kernel_function.h_params()[_kernel_function.h_params_size() - 1]);

            // O(n^2) [should be negligible]
            for (size_t i = 0; i < n; i++)
                for (size_t j = 0; j <= i; ++j)
                    _kernel(i, j) = _kernel_function(_samples[i], _samples[j]) + ((i == j) ? _noise + 1e-6 : 0); // noise only on the diagonal
        ...
        void _compute_incremental_kernel()
        {
            ...
            for (size_t i = 0; i < n; ++i) {
                _kernel(i, n - 1) = _kernel_function(_samples[i], _samples[n - 1]) + ((i == n - 1) ? _noise + 1e-6 : 0); // noise only on the diagonal
